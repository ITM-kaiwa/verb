  ...
  if (phase === "summary") {
    return (
      <div className="mx-auto max-w-md space-y-4 rounded-2xl border border-sand-300 bg-sand-50 p-8 text-center shadow-card">
        <p className="text-lg font-semibold text-sand-700">ゲーム終了！</p>
        <p className="text-sand-600">
          {score}/{TOTAL_ROUNDS} 問正解しました。
        </p>
        <button
          type="button"
      ...
      <div className="flex flex-col gap-4 rounded-3xl border border-lemon-300/70 bg-lemon-100 p-5 shadow-card sm:flex-row">
        ...
        <div className="flex flex-1 flex-col gap-2">
          <p className="text-center text-[11px] font-medium text-sand-600 sm:text-left">
            落ちてくる動詞の「{formLabel?.labelJa}」と同じものを選んでください
          </p>
          ...
          {phase === "result" && result && (
            ...
            >
              {result.timedOut && <p className="font-semibold">時間切れ！</p>}
              {!result.timedOut && result.success && <p className="font-semibold">正解！</p>}
              {!result.timedOut && !result.success && (
                <p className="font-semibold">
                  不正解。正解は「{roundData.correctAnswer}」でした。
                </p>
              )}
              <button
                type="button"
